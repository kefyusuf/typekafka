...
# Requires Docker + Docker Compose and that no leftover nodejs-kafka
...
for i in $(seq 1 40); do
  H=$(docker inspect --format '{{.State.Health.Status}}' nodejs-kafka-kafka-secured 2>/dev/null)
  [ "$H" = "healthy" ] && break
  sleep 5
done
...
for i in $(seq 1 12); do
  S=$(docker inspect --format '{{.State.Status}}' nodejs-kafka-security-init 2>/dev/null)
  [ "$S" = "exited" ] && break
  sleep 5
done
...
for i in $(seq 1 30); do
  S=$(docker inspect --format '{{.State.Status}}' nodejs-kafka-producer-secured 2>/dev/null)
  [ "$S" = "exited" ] && break
  sleep 5
done

echo "=== producer-secured logs ==="
docker logs nodejs-kafka-producer-secured
echo "=== consumer-secured logs (tail) ==="
docker logs nodejs-kafka-consumer-secured | tail -25

echo "=== ACL LIST (admin) ==="
MSYS_NO_PATHCONV=1 docker exec nodejs-kafka-kafka-secured /opt/kafka/bin/kafka-acls.sh \
...
MSYS_NO_PATHCONV=1 docker exec nodejs-kafka-kafka-secured /opt/kafka/bin/kafka-console-consumer.sh \
